...
import { START_BEFORE, CHECK_ITEMS, DROPDOWN_OPTIONS } from "@/shared/constant/recipients";
import { thanksLetterData } from "@/features/recipients/mock-data";
...
export default function Letters() {
  ...
  const sortedData = [...thanksLetterData].sort(
    ...
    <>
      ...
      >
        ...
        <div className={clsx("flex items-end justify-between", "mobile:items-center")}>
          ...
          <Button size={isMobile ? "small" : "large"} children="수혜자 편지쓰기" />
        </div>
          ...
          >
            {sortedData.slice(0, letterCount).map((item, index) => (
              <LetterCard
                ...
                views={item.views}
                onClick={() => navigate(`/remembrance/recipients-view/${item.letterNumber}`)} // 추가
              />
            ))}
          </div>
          <Button
