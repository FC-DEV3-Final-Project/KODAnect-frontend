...
function Depth2Menu({ items, selectedDepth2, onClick }: Depth2MenuProps) {
  ...
  return (
    <ul className="min-w-[280px] py-p3 pr-p8">
      {items.map((item, index) => {
        ...
        return (
          <li key={index} onClick={() => onClick(index)}>
            {item.children ? (
              <button type="button" className={clsx(menuStyle, isSelected && menuActiveStyle)}>
                {item.label}
                <img src={Arrow} alt="메뉴 더보기 아이콘" className={iconStyle} />
              </button>
            ) : (
              <Link to={item.path!} className={clsx(menuStyle, isActive && menuActiveStyle)}>
                {item.label}
              </Link>
            )}
          </li>
